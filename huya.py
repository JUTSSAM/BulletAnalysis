...
def connect():
    print '-----*-----Huya_Spider-----*-----\n'
    host = socket.gethostbyname("7522141e-ws.va.huya.com")
    port = 8601
    global s
...
def danmu(room_id):
    ...
    while True:
        ...
        if judge_chatmsg(content):
            ...
            # textrank_s = " ".join(jieba.analyse.textrank(chatmsg.decode('utf-8')))
            senti = SnowNLP(chatmsg).sentiments
            print '[%s] : %s -- sentiment: %s' % (nickname, chatmsg, senti)
        else:
            pass
...
if __name__ == '__main__':
    connect()
    # t1 = threading.Thread(target=danmu, args=(606118,))
